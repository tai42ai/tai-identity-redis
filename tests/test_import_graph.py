...
``tai_identity_redis`` is on the allowlist. The rule (see the README): the shipped
package imports
the redis client, the shared platform substrate (tai-contract + tai-kit[redis],
pydantic-settings) and their dependency closure ONLY,
...
The runtime walk imports ``tai_identity_redis`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai_identity_redis`` means the assertion covers the SHIPPED package's
...
PACKAGE = "tai_identity_redis"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai_contract", "tai_kit"})

# Every third-party root the shipped ``tai_identity_redis`` graph pulls in -- the declared runtime dependencies plus
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai_identity_redis graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai_identity_redis module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai_identity_redis sources: {offenders}"
