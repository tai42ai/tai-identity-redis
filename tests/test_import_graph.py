...
``tai42_identity_redis`` is on the allowlist. The rule (see the README): the shipped
package imports
the redis client, the shared platform substrate (tai42-contract + tai42-kit[redis],
...
The runtime walk imports ``tai42_identity_redis`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai42_identity_redis`` means the assertion covers the SHIPPED package's
...
# Every third-party root the shipped ``tai42_identity_redis`` graph pulls in -- the declared runtime dependencies plus
