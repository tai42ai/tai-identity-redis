...
def test_readiness_targets_declares_the_redis_record_store(monkeypatch):
    ...
    the ``"access_control"`` label, kit's ``RedisClient``, and this provider's redis
    settings — the exact connection core previously hard-coded, so behavior is
    unchanged while core stays generic."""
    ...
    # The declared settings are the provider's own redis settings — the value core
    # opens and pings, not a copy.
