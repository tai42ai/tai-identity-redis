...
class FakeRedis:
    ...
    def __init__(
        ...
        *,
        ...
        # A one-shot callback fired just before the first pipeline EXEC (after the
        # pipeline has WATCHed and read), used to simulate a concurrent client
        # committing inside the WATCH/EXEC window: that EXEC then sees the watched key
        # changed and raises ``WatchError``, driving the retry. Cleared after it fires
        # so the retry runs clean.
    ...
    async def hgetall(self, key):
        ...
        # Decoded-responses shape: a fresh field/value string map, ``{}`` for a
        # missing key. Copy so an in-place edit never leaks into the store.
        return dict(self._hashes.get(key, {}))

    async def hset(self, key, mapping):
        # Mirrors ``client.hset(key, mapping=mapping)`` — merge the string-valued
        # fields into the key's hash and return the count of newly added fields.
    ...
    async def scan_iter(self, match):
        ...
        # ``fnmatch`` is the faithful stand-in.
        seen: set[str] = set()
...
class _FakePipeline:
    """An in-memory stand-in for a redis-py async pipeline with WATCH/MULTI: after
    ``watch`` the pipeline's reads run against live state and its writes are queued,
    and ``execute`` applies the queued writes only if no watched key changed since
    ``watch`` (otherwise it raises ``WatchError``, as redis does on a broken watch)."""
    ...
    def multi(self) -> None:
        # No-op: this fake queues every write regardless, so there is no separate
        # buffered mode to switch into; multi() exists as the call site provision uses.
        return None
    ...
    async def execute(self, raise_on_error: bool = True) -> list:
        # A concurrent write landing in the WATCH/EXEC window fires here, just before
        # the watch comparison, so this EXEC observes the changed key and aborts.
